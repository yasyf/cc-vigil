...
/// daemon's confirm loop and the app's socket client both size their timeouts from
/// one source. A slow pmset can make the daemon spend up to
/// `attempts × helperCallSeconds` confirming the clear; the client sits strictly
/// above that so a slow-but-progressing clear is
...
public enum ClearBudget {
    ...
    public static let clientSeconds = Int(daemonWorstCaseSeconds) + 20
}
